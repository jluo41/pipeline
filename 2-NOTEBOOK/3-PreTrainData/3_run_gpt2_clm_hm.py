...
from nn.cgmdecoder.configuration_cgmdecoder import GPT2WithHMConfig
from nn.cgmdecoder.modeling_cgmdecoder import GPT2WithHMForCausalLM


HFDataName = 'PreTrainBench-CausalLM-Split-v0515'
...
model_name = 'gpt2-cgm-clm-with-hm'
config = GPT2WithHMConfig(
...
model = GPT2WithHMForCausalLM(config=config)
...
def compute_metrics(pred):
    ...
    # Ignore loss on padding tokens (label == -100)
    loss_fct = torch.nn.CrossEntropyLoss(ignore_index=-100)
    next_token_loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

    # Get predictions
    predictions = torch.argmax(shift_logits, dim=-1)

    # Compute accuracy only where label is not -100
    ...
    return {
        ...
        # 'num_evaluated_tokens': mask.sum().item()
    }
...
export WANDB_PROJECT=CLM-GPT2
# python 2-NOTEBOOK/3-PreTrainData/3_run_gpt2_clm_hm.py
python 2-NOTEBOOK/3-PreTrainData/3_run_gpt2_clm_hm.py; exit
